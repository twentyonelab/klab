...
Nowe w v0.11 — pięć żądań Krzyśka wokół edycji planu i twardej zasady kolejności procesu: **(1) Przesuwalne ciągi** — ciągi komunikacyjne mają teraz `cid`, klasę `.ciag` (dane `data-cid`), są zaznaczalne i przeciągalne jak strefy (`dragC`), z kartą po prawej („Wybrany ciąg komunikacyjny": długość/szerokość/usuń) i podpowiedziami szerokości wg BHP. **(2) Zmiana rozmiaru chwytem za krawędź** — zaznaczony prostokąt (ciąg LUB strefa) dostaje **8 uchwytów** (`handlesSVG`, narożniki + środki boków, kursory `nwse/nesw/ns/ew-resize`) rysowanych w warstwie na wierzchu; `resizing` w pointermove przelicza x/y/w/h (min 1 m, snap do siatki), przeciwległa krawędź trzymana ze stanu startowego. **(3) „Optymalizuj" tylko optymalizuje** — NIE otwiera (ani nie zostawia otwartego) okna Auto-układu; wynik idzie do lekkiego **toasta** (`#optToast`, dół-środek, z ✕ i ewentualnym „⤢ Powiększ halę teraz"). **(4) Naprawiony drag przy zoomie** — `toSVG()` przeliczany przez `getScreenCTM().inverse()` (uwzględnia viewBox + preserveAspectRatio), więc kafelek trzyma się kursora ściśle przy każdym przybliżeniu; pan też liczony z CTM. **(5) Kolejność procesu = twarda zasada w Auto-układzie** — `procesCheck()` sprawdza „linia A nad B" + monotoniczność łańcucha we→wy w każdej linii (dopuszcza serpentynę). Każda z 3 propozycji ma badge ✓/⚠; „Kompakt" dostaje **⚠ „łamie kolejność procesu"** z wyjaśnieniem, a NAJLEPSZA jest wybierana najpierw spośród wariantów z zachowaną kolejnością. `applyStrat` i `optymalizuj` też raportują status kolejności (✓/⚠). Audyt Playwright (`verify11.mjs`): **53× PASS**, dd/dz/cd/cc=0, round-trip `toSVG` dokładny, drag/resize ciągu potwierdzony end-to-end przy zoomie. (v0.10 zostaje w repo jako archiwum.)

## Stan poprzedni (v0.10)
